        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header with-border">
                    <form class="form-inline" id="form-list-admins" method="POST" action="{{ route('api.admin.list') }}">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="hidden" class="page" name="page" value="1"/>
                        <input type="hidden" class="page-size" name="page_size" value="20"/>
                        <div class="form-group">
                            <input type="text" name="name" class="form-control" placeholder="输入要查询的姓名">
                        </div>
                        <span class="btn btn-primary" id="btn-search">筛选</span>

                        <a class="btn btn-primary pull-right" href="{{ route('admin.admin.create') }}"><i class="fa fa-plus"></i> 新增管理员</a>
                    </form>
                </div>
                <div class="box-body">
                    <div class="row">
...
<script id="tpl-list" type="javascript/template">
    ...
    <tr>
        ...
        <td class="text-center">
            <a href="{{ url('admin/admin/detail') }}/@{{ admin.id }}" class="btn btn-info btn-xs">详情</a>
            <a href="{{ url('admin/admin/update') }}/@{{ admin.id }}" class="btn btn-warning btn-xs btn-update">更新</a>
            <span class="btn btn-danger btn-xs btn-remove" data-url="{{ url('api/admin/delete') }}?id=@{{ admin.id }}">
                删除
            </span>
        </td>
    </tr>
